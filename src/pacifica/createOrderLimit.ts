// Create limit order on Pacifica using agent wallet signature
...
export async function createLimitOrder(params: LimitOrderParams): Promise<LimitOrderResponse> {
  try {
    logger.info(`Creating limit order: ${params.side} ${params.amount} ${params.symbol} @ ${params.price}`);
    ...
    // Sign the message
    const message = prepareMessage('create_order', timestamp, expiryWindow, orderPayload);
    ...
    logger.debug('Sending limit order request to Pacifica API');

    // Send request
    const response = await axios.post(
      `${config.pacificaBaseUrl}/orders/create`,
      request,
      {
        headers: {
    ...
    logger.info(`Limit order created successfully: order_id=${response.data.data.order_id}`);
    ...
    logger.error('Failed to create limit order:', error.response?.data || error.message);
